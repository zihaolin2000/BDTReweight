...
from BDTReweight.analysis import transform_momentum_to_reaction_frame, create_dataframe_from_nuisance, draw_source_target_distributions_and_ratio, draw_2Dxsec_and_efficiency
...
def load_reweighter(category):
    # The same reweighters as those used in NumuTKI MINERvA-101-Cross-Section:
    with open(f'/exp/minerva/data/users/zihaolin/BDTReweighters/saved_reweighters_pickle/{train_size}/reweighter_MINERvA_ME_numuCarbon_CCQELike_GENIEv2_to_v3AR23_1mu{category}.pkl', 'rb') as f:
...
variable_to_extract = {
    ...
    '1p0n':['Enu_true', 'Q2', 'q0', 'q3', 'W',
            ...
            'leading_muon_px', 'leading_muon_py', 'leading_muon_pz', 'leading_muon_KE',
            'total_proton_px', 'total_proton_py', 'total_proton_pz', 'total_proton_KE',
            'leading_proton_KE', 'leading_proton_theta',
...
drawing_variables_cat = {
    '0p0n':['total_proton_px','total_proton_py','total_proton_pz',
            'total_proton_KE','leading_muon_py','leading_muon_pz',
            'weight'],
    '0pNn':['total_proton_px','total_proton_py','total_proton_pz',
            'total_proton_KE','leading_muon_py','leading_muon_pz', 
            'leading_neutron_px', 'leading_neutron_py', 'leading_neutron_pz',
            'weight'],
    '1p0n':['leading_proton_px','leading_proton_py','leading_proton_pz',
            'total_proton_KE','leading_muon_py','leading_muon_pz', 
            'dpt', 'dalphat', 'dphit', 
            'leading_proton_KE', 'leading_proton_theta', 'weight'],
    '1pNn':['leading_proton_px','leading_proton_py','leading_proton_pz',
            'total_proton_KE','leading_muon_py','leading_muon_pz',
            'leading_neutron_px','leading_neutron_py','leading_neutron_pz',
            'dpt', 'dalphat', 'dphit',
...
legends = ['GENIE v2.12.6 ($v2$)', 'GENIE v2.12.6 reweighted ($v2\'$)', 'GENIE v3.04.00 AR23_20i_00_000 ($v3$)']
...
for category in categories:
    ...
    reweighter = load_reweighter(category)
...
# ============= companion plots when weights with >10, >5, >3 are dropped ==============
print('plot companion plots when weights with >10, >5, >3 are dropped...')

mask = source_test_all['weight'] <= 10
draw_source_target_distributions_and_ratio(source_test_all[mask], target_test_all, 
        variables = drawing_variables,legends = legends,
        source_weights = np.ones(len(source_test_all[mask])),
        target_weights = np.ones(len(target_test_all)), 
        ...
        ylabels = [diff_xsec_latex_wrt_variable(var) for var in drawing_variables],
        scale_source = scale_source_test, scale_target = scale_target_test,
        savepath = '/exp/minerva/app/users/zihaolin/REWEIGHTworkdir/ZihaoWD/plots/combined-sumProton-test-weight-0-10.png',
        figshow = False
)

mask = source_test_all['weight'] <= 5
draw_source_target_distributions_and_ratio(source_test_all[mask], target_test_all, 
        variables = drawing_variables,legends = legends,
        source_weights = np.ones(len(source_test_all[mask])),
        target_weights = np.ones(len(target_test_all)), 
        ...
        ylabels = [diff_xsec_latex_wrt_variable(var) for var in drawing_variables],
        scale_source = scale_source_test, scale_target = scale_target_test,
        savepath = '/exp/minerva/app/users/zihaolin/REWEIGHTworkdir/ZihaoWD/plots/combined-sumProton-test-weight-0-5.png',
        figshow = False
)

mask = source_test_all['weight'] <= 3
draw_source_target_distributions_and_ratio(source_test_all[mask], target_test_all, 
        variables = drawing_variables,legends = legends,
        source_weights = np.ones(len(source_test_all[mask])),
        target_weights = np.ones(len(target_test_all)), 
        ...
        ylabels = [diff_xsec_latex_wrt_variable(var) for var in drawing_variables],
        scale_source = scale_source_test, scale_target = scale_target_test,
        savepath = '/exp/minerva/app/users/zihaolin/REWEIGHTworkdir/ZihaoWD/plots/combined-sumProton-test-weight-0-3.png',
        figshow = False
)


# ======================== Plot 2D cross section and efficiency ========================
print('Plot 2D xsec and efficiency')
draw_2Dxsec_and_efficiency(
        df_genie2=source_test_leading_proton,df_genie3=target_test_leading_proton,
        fScale_genie2=scale_source_test,fScale_genie3=scale_target_test,
        savepath_xsec='/exp/minerva/app/users/zihaolin/REWEIGHTworkdir/ZihaoWD/plots/NMEff-test.png',
        savepath_ratio='/exp/minerva/app/users/zihaolin/REWEIGHTworkdir/ZihaoWD/plots/Eff2Eff3-step-test.png',
        figshow = False,
)

# ================== Plot total xsec of each category ==================
print('Plot total xsec by category')
# categories = ['0p0n', '0pNn', '1p0n', '1pNn', '2p0n', '2pNn', 'others']
categories = ['1p0n', 'others','1pNn', '2pNn', '0pNn', '0p0n', '2p0n']
source_xsec = []
target_xsec = []
for category in categories:
    source_xsec.append(len(source_test[category]) * scale_source_test)
    target_xsec.append(len(target_test[category]) * scale_target_test)

plt.figure(figsize=(8, 5), dpi = 300)
plt.bar(categories, source_xsec, label='GENIE v2.12.6', color='orange',alpha=0.5)
plt.bar(categories, target_xsec, label='GENIE v3.04.00 AR23_20i_00_000', color='deepskyblue', alpha=0.5)
...
# plt.title('Cross-secion by Categories')
plt.legend(frameon=False)
plt.tight_layout()
plt.savefig('/exp/minerva/app/users/zihaolin/REWEIGHTworkdir/ZihaoWD/plots/categories_histogram.png')
plt.close()
